	...
	{#if settings}
		...
		<form use:enhance method="post" action="?/update" class="flex flex-col gap-3 w-1/3">
			<div class="flex justify-between text-lg">
				...
				<input type="text" id="eventName" name="eventName" value={settings.eventName} class="border-b border-gray-400 focus:border-gray-900 focus:outline-none" />
			</div>
			<div class="flex justify-between text-lg">
				<label for="description">Event Description</label>
				<input id="description" type="text" name="description" value={settings.description} class="border-b border-gray-400 focus:border-gray-900 focus:outline-none" />
			</div>
			<div class="flex justify-between text-lg">
            ...
            <button class="rounded p-2 bg-indigo-200 hover:bg-indigo-300 drop-shadow">Update Event</button>
		</form>
	{/if}
