...
export const POST = async ({ request, locals: { supabase } }: { request: any; locals: { supabase: SupabaseClient } }) => {
	//pull eventId from param
	const body = await request.json();
	const slug = body.slug;
	const answers = JSON.parse(body.answers) as Answer[];
	// Get questions
	const { formId, questions, eventId } = await getFormInfo(supabase, slug);
	...
	await incrementSubmissions(supabase, eventId);
...
async function getFormInfo(supabase: SupabaseClient, slug: string) {
	const { data, error } = await supabase.from("events").select("currentFormId, formQuestions, id").eq("slug", slug);

	if (error) throw new Error(error.message);
	return { formId: data[0].currentFormId, questions: data[0].formQuestions as Question[], eventId: data[0].id };
}
...
async function incrementSubmissions(supabase: SupabaseClient, eventId: string) {
	console.log("adding data to " + eventId)
	const { data, error } = await supabase.rpc("incrementSubmission", { formId: eventId });
	if (error) throw new Error(error.message);
}
